...
export function remotePayload(entry: OutboxEntry) {
  ...
  if (entry.entity === 'item') {
    ...
    switch (payload.kind) {
      ...
      case 'stay': return { ...base, day_id: payload.dayId || null, start_time: payload.startTime || null, display_status: payload.status || null, name: payload.title, location: payload.location || null, check_in: null, check_out: null, maps_url: payload.mapsUrl || null, booking_id: null, notes: payload.description, planned_amount: payload.plannedAmount ?? null, actual_amount: payload.actualAmount ?? null, currency: payload.currency ?? null, position: payload.position }
      ...
      case 'place': return { ...base, day_id: payload.dayId || null, start_time: payload.startTime || null, category: 'place', name: payload.title, location: payload.location || null, maps_url: payload.mapsUrl || null, notes: payload.description, status: payload.status || 'saved', position: payload.position }
      case 'food': return { ...base, day_id: payload.dayId || null, start_time: payload.startTime || null, category: 'restaurant', name: payload.title, location: payload.location || null, maps_url: payload.mapsUrl || null, notes: payload.description, status: payload.status || 'saved', position: payload.position }
      case 'note': return { ...base, day_id: payload.dayId || null, start_time: payload.startTime || null, note_type: 'note', title: payload.title, body: payload.description, priority: 0, position: payload.position }
