...
function withBudgetDefaults(trip: Trip): Trip {
  const displayCurrency = normalizeCurrency(
    ...
    normalizeCurrency(trip.baseCurrency)
  )
  return {
    ...trip,
    displayCurrency,
...
export const localRepository = {
  ...
  async saveTrip(
    input: Partial<Trip> & Pick<Trip, 'title' | 'startDate' | 'endDate'>
  ): Promise<Trip> {
    const existing = input.id ? await db.trips.get(input.id) : undefined
    const timestamp = now()
    const trip: Trip = {
      id: input.id ?? crypto.randomUUID(),
      ownerId: input.ownerId ?? existing?.ownerId ?? 'local-owner',
      title: input.title,
      subtitle: input.subtitle ?? existing?.subtitle ?? '',
      startDate: input.startDate,
      endDate: input.endDate,
      timezone: input.timezone ?? existing?.timezone ?? 'UTC',
      baseCurrency: input.baseCurrency ?? existing?.baseCurrency ?? 'USD',
      displayCurrency: normalizeCurrency(
        input.displayCurrency ?? existing?.displayCurrency ?? input.baseCurrency
      ),
      budgetAmount: input.budgetAmount ?? existing?.budgetAmount ?? 0,
      budgetCurrency: normalizeCurrency(
      ...
      coverAlt:
        input.coverAlt ?? existing?.coverAlt ?? 'Mountain landscape seen from a train window',
      status: input.status ?? existing?.status ?? 'upcoming',
      shareEnabled: input.shareEnabled ?? existing?.shareEnabled ?? false,
  ...
  async duplicateTrip(tripId: string): Promise<Trip> {
    ...
    const trip = await this.saveTrip({
      ...bundle.trip,
      id: undefined,
    ...
    for (const item of bundle.items) {
      await this.saveItem({
        ...
        id: undefined,
        tripId: trip.id,
        dayId: item.dayId ? dayMap.get(item.dayId) : undefined
      })
    }
    return trip
  },
  async saveDay(
    input: Partial<TripDay> & Pick<TripDay, 'tripId' | 'date' | 'title'>
  ): Promise<TripDay> {
    ...
    const day: TripDay = {
      ...
      baseLocation: 'baseLocation' in input ? input.baseLocation : existing?.baseLocation,
      position:
        input.position ??
        existing?.position ??
        (await db.days.where('tripId').equals(input.tripId).count()),
      createdAt: existing?.createdAt ?? timestamp,
      updatedAt: timestamp,
      version: (existing?.version ?? 0) + 1,
      deletedAt: existing?.deletedAt
  ...
  async duplicateDay(day: TripDay): Promise<TripDay> {
    const duplicate = await this.saveDay({
      ...day,
      id: undefined,
      title: `${day.title} copy`,
      position: day.position + 0.5
    })
    const items = (await db.items.where('dayId').equals(day.id).toArray()).filter(
      (item) => !item.deletedAt
    )
  ...
  async moveDay(day: TripDay, delta: -1 | 1, siblings: TripDay[]) {
    ...
    if (!swap) return
    await Promise.all([
      this.saveDay({ ...day, position: swap.position }),
      this.saveDay({ ...swap, position: day.position })
    ])
  },
  async saveItem(
    input: Partial<ContentItem> & Pick<ContentItem, 'tripId' | 'kind' | 'title'>
  ): Promise<ContentItem> {
    ...
    const item: ContentItem = {
      ...
      emailUrl: input.emailUrl,
      latitude: input.latitude,
      longitude: input.longitude,
      geocodedLocation: input.geocodedLocation,
      originLatitude: input.originLatitude,
      originLongitude: input.originLongitude,
      originGeocodedLocation: input.originGeocodedLocation,
      destinationLatitude: input.destinationLatitude,
      destinationLongitude: input.destinationLongitude,
      destinationGeocodedLocation: input.destinationGeocodedLocation,
      ...
      position:
        input.position ??
        existing?.position ??
        (await db.items.where('tripId').equals(input.tripId).count()),
  ...
  async moveItem(item: ContentItem, delta: -1 | 1, siblings: ContentItem[]) {
    ...
    if (!swap) return
    await Promise.all([
      this.saveItem({ ...item, position: swap.position }),
      this.saveItem({ ...swap, position: item.position })
    ])
  },
  async reorderItems(activeId: string, overId: string, siblings: ContentItem[]) {
    ...
    for (let position = 0; position < ordered.length; position += 1) {
      if (ordered[position].position !== position)
        await this.saveItem({ ...ordered[position], position })
    }
  },
  ...
  async queuePhoto(tripId: string, blob: Blob, altText: string): Promise<MediaRecord> {
    ...
    await enqueue(
      ...
      media.id,
      'upload',
      { ...media, blob: undefined, size: blob.size, type: blob.type },
      0
    )
    return media
  },
  async exportTrip(tripId: string): Promise<string> {
    ...
    return JSON.stringify(
      { format: 'roam-trip', version: 1, exportedAt: now(), data: bundle },
      null,
      2
    )
  },
  async importTrip(raw: string): Promise<Trip> {
    ...
    if (parsed.format !== 'roam-trip' || !parsed.data?.trip)
      throw new Error('Choose a valid Roam JSON export.')
    const newId = crypto.randomUUID()
    const trip = await this.saveTrip({
      ...parsed.data.trip,
      id: newId,
      title: `${parsed.data.trip.title} imported`,
      shareEnabled: false
    })
    const dayMap = new Map<string, string>()
    ...
    for (const source of parsed.data.items ?? []) {
      await this.saveItem({
        ...
        id: undefined,
        tripId: newId,
        dayId: source.dayId ? dayMap.get(source.dayId) : undefined
      })
    }
    return trip
  },
  async pendingCount(tripId?: string) {
    const entries = tripId
      ? await db.outbox.where('tripId').equals(tripId).toArray()
      : await db.outbox.toArray()
    return entries.length
  },
  async replaceFromCloud(bundle: {
    trips: Trip[]
    days: TripDay[]
    items: ContentItem[]
    media: MediaRecord[]
