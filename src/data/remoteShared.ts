...
export async function getRemoteSharedTrip(rawToken: string): Promise<TripBundle | undefined> {
  ...
  for (const row of rows(payload.itinerary_items)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'activity', title: text(row.title), description: text(row.description), startTime: text(row.start_time) || undefined, endTime: text(row.end_time) || undefined, location: text(row.location_name) || undefined, mapsUrl: text(row.maps_url) || undefined, status: text(row.status) || undefined })
  for (const row of rows(payload.bookings)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'booking', title: text(row.title), description: text(row.notes), startTime: text(row.start_time) || undefined, location: text(row.location_name) || undefined, mapsUrl: text(row.maps_url) || undefined, provider: text(row.provider) || undefined, status: text(row.display_status ?? row.status) || undefined })
  for (const row of rows(payload.stays)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'stay', title: text(row.name), description: text(row.notes), startTime: text(row.start_time) || undefined, location: text(row.location) || undefined, mapsUrl: text(row.maps_url) || undefined, status: text(row.display_status) || undefined })
  for (const row of rows(payload.transports)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'transport', title: text(row.title), description: text(row.notes), startTime: text(row.start_time) || undefined, provider: text(row.provider) || undefined, origin: text(row.origin) || undefined, location: text(row.destination) || undefined, status: text(row.display_status ?? row.status) || undefined })
  for (const row of rows(payload.route_stops)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), kind: 'route', title: text(row.city), description: text(row.notes), location: text(row.city), mapsUrl: text(row.maps_url) || undefined })
  for (const row of rows(payload.places)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'place', title: text(row.name), description: text(row.notes), startTime: text(row.start_time) || undefined, location: text(row.location) || undefined, mapsUrl: text(row.maps_url) || undefined, status: text(row.status) || undefined })
  for (const row of rows(payload.food_nightlife)) items.push({ ...base(row), ...costFields(row), ...mapFields(row), dayId: text(row.day_id) || undefined, kind: 'food', title: text(row.name), description: text(row.notes), startTime: text(row.start_time) || undefined, location: text(row.location) || undefined, mapsUrl: text(row.maps_url) || undefined, status: text(row.status) || undefined })
